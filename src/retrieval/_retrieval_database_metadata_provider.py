...
class RetrievalDatabaseMetadataProvider:
    ...
    def __init__(self, metadata_dir: str, getitem_only_caption: bool = False) -> None:
        metadatas = [
            str(a)
            for a in sorted(Path(metadata_dir).glob("**/*"))
            if a.is_file() and os.path.splitext(a)[-1] == ".arrow"
        ]
    ...
    def get(self, ids: list[int]) -> pa.Table:
        ...
        t = pa.concat_tables(
            [self.table[start:end] for start, end in zip(ids, end_ids, strict=False)]
        )
        return t.select(columns).to_pandas().to_dict("records")

    def __getitem__(self, key: slice | int | list) -> list[dict] | dict:
