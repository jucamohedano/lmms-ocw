...
from ._retrieval_database_metadata_provider import RetrievalDatabaseMetadataProvider
...
class RetrievalDatabase:
    ...
    def _map_to_metadata(
        ...
        for key, (d, i, emb) in enumerate(zip(distances, indices, embs, strict=False)):
            output = {}
    ...
    def _meta_to_dict(self, metadata: dict) -> dict:
    ...
    def _get_connected_components(self, neighbors: dict) -> list:
    ...
    def _deduplicate_embeddings(self, embeddings: np.matrix, threshold: float = 0.94) -> set:
        ...
        _l, _, indices = index.range_search(embeddings, threshold)
    ...
    def query(
        ...
        ----
            ...
            return_metadata (bool): Whether to return metadata. Default is True.
            deduplication (bool): Whether to deduplicate results. Default is True.
            deduplication_skip_first (int): Number of initial results to skip for deduplication
                (to avoid removing very similar images). Default is 0.
            deduplication_threshold (float): Threshold to use for deduplication. Default is 0.94.
        ...
        for i in range(len(results)):
            ...
            if deduplication:
                ...
                local_indices_to_remove = self._deduplicate_embeddings(
                    result_embeddings, threshold=deduplication_threshold
                )
            else:
            ...
            for ind, dis, emb in zip(
                result_indices, result_distances, result_embeddings, strict=False
            ):
