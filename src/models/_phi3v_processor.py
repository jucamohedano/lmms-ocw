...
from ._phi3v_image_processing import Phi3VImageProcessor

transformers.Phi3VImageProcessor = Phi3VImageProcessor


class Phi3VProcessor(ProcessorMixin):
    r"""Constructs a Phi3-V processor which wraps a Phi3-V image processor and a LLaMa tokenizer into a single processor.
    ...
    def _convert_images_texts_to_inputs(
        ...
        for text in texts:
            prompt_chunks.append(
                [self.tokenizer(chunk).input_ids for chunk in re.split(pattern, text)]
            )
        ...
        # check the condition
        assert (
            unique_image_ids == list(range(1, len(unique_image_ids) + 1))
        ...
        def insert_separator(X, sep_list):
            if len(X) > len(sep_list):
                sep_list.append([])
            return [ele for sublist in zip(X, sep_list, strict=False) for ele in sublist]

        input_ids = []
        for sub_prompt_chunks, sub_image_ids_pad in zip(
            prompt_chunks, image_ids_pad, strict=False
        ):
            input_ids.append([])
            offset = 0
    ...
    def model_input_names(self) -> list[dict]:
