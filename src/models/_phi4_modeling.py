...
class Phi4MMImageEmbedding(nn.Module):
    ...
    def __init__(self, config: PretrainedConfig, **kwargs) -> None:
        ...
        self.image_token_compression_cls = kwargs.get("image_token_compression_cls")
        ...
        if self.with_learnable_separator:
            ...
            self.sub_GN = nn.Parameter(
                torch.zeros([1, 1, 1, self.image_dim_out * self.base_feat_height_reduction**2])
            )
            logger.info(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if fake_image_forward and self.training:
            hidden_states = (
                hidden_states
                + (0 * img_set_tensor[0].to(hidden_states.dtype).to(hidden_states.device)).sum()
            )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if len(positions.tolist()) > 0:
            ...
            if self.training:
                audio_embeds = (
                    torch.zeros(1, 500, self.audio_dim_in).to(target_device).to(target_dtype)
                )
        ...
        if len(positions.tolist()) > 0:
            ...
            if self.training:
                hidden_states = (
                    hidden_states
                    + (
                        0 * audio_set_tensor[:, 0].to(hidden_states.dtype).to(hidden_states.device)
                    ).sum()
                )
...
class Phi4MMImageAudioEmbedding(nn.Module):
    ...
    def forward(
        ...
        input_ids: torch.LongTensor,
        input_embeds,
        input_image_embeds: torch.FloatTensor | None = None,
        input_audio_embeds: torch.FloatTensor | None = None,
        image_sizes=None,
        image_attention_mask=None,
        audio_embed_sizes=None,
        audio_attention_mask=None,
...
class Phi4MMAttention(nn.Module):
    ...
    def __init__(self, config: Phi4MMConfig, layer_idx: int | None = None):
        super().__init__()
        self.config = config
