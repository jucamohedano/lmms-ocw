...
class EmbedderGME(Model):
    """GME Model, used to embed stuff.
    ...
    def __init__(
        self,
        model_name_or_path: str = "Alibaba-NLP/gme-Qwen2-VL-7B-Instruct",
        use_cache: bool = True,
    ...
    def load_model(self) -> None:
        ...
        self._model = AutoModel.from_pretrained(
            self._model_name_or_path, trust_remote_code=True, **model_kwargs
        )
        self._model.eval()
    ...
    def generate_until(self, requests: list[TaskInstance]) -> list[str]:
        ...
        for chunk in chunks:
            ...
            with torch.no_grad():
                embeddings = self._model.get_image_embeddings(images=batched_visuals)
...
@register_model("qwen2-vl-7b-gme")
def qwen2_vl_7b_gme(**model_kwargs) -> Model:
    """Load the Qwen2 VL 7B GME model."""
    model_name_or_path = "Alibaba-NLP/gme-Qwen2-VL-7B-Instruct"
    model = EmbedderGME(model_name_or_path, **model_kwargs)
    return model
