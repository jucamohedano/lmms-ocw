...
class Phi4MMImageProcessor(BaseImageProcessor):
    r"""Constructs a Phi4MM image processor."""
    ...
    def dynamic_preprocess(
        self, image, min_num=1, max_num=12, image_size=384, mask_size=27, use_thumbnail=True
    ):
    ...
    def preprocess(
        ...
        hd_images_reshape = [
            im.reshape(
                1, 3, h // base_resolution, base_resolution, w // base_resolution, base_resolution
            )
            .permute(0, 2, 4, 1, 3, 5)
        ...
        attention_masks_reshape = [
            mask.reshape(
                1, h // mask_resolution, mask_resolution, w // mask_resolution, mask_resolution
            )
            .permute(0, 1, 3, 2, 4)
        ...
        hd_masks_reshape = [
            ...
            for _global_mask, _mask in zip(
                global_attention_mask, attention_masks_reshape, strict=False
            )
        ]
...
AudioInput = tuple[np.ndarray | torch.Tensor, int]
...
class Phi4MMProcessor(ProcessorMixin):
    r"""Constructs a Phi4MM processor which raps an image processor, a audio processor, and a GPT tokenizer into a single processor.
    ...
    def _convert_images_audios_text_to_inputs(
        ...
        processed_text = [
            re.sub(_COMPATIBLE_IMAGE_SPECIAL_TOKEN_PATTERN, _IMAGE_SPECIAL_TOKEN, t) for t in text
        ]
        processed_text = [
