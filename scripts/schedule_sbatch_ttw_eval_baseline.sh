...
# qwen2.5-vl-7b uses its default (Qwen/Qwen2.5-VL-7B-Instruct).
if [[ -n "${MODEL_NAME_OR_PATH}" ]]; then
    model_args_line="    --model_args model_name_or_path=${MODEL_NAME_OR_PATH} \\"
else
    model_args_line="    \\"
fi
...
# -----------------------------------------------------------
# Baseline evaluation (no TTW): Qwen2.5-VL-7B direct inference.
#
# Mirrors schedule_sbatch_ttw_eval_lora.sh SLURM layout (array jobs, resources,
# limit, wandb, accelerate) but without TTW / offline captions / LoRA args.
# -----------------------------------------------------------
...
echo "Baseline evaluation finished for \${task}. Results in: \${EVAL_OUTPUT_DIR}"
EOT
